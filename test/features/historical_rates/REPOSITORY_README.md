...
**الهدف:** نتأكد ان الـ Repository بيمرر الـ rates من الـ Data Source بنجاح

**المثال:**
```
Data Source رجع: [HistoricalRateModel, HistoricalRateModel]
المفروض Repository يرجع: Right([HistoricalRateModel, HistoricalRateModel]) ✅
```

**الكود:**
```dart
test('should return Right when data source succeeds', () async {
  // Arrange - نخلي الـ mock data source يرجع rates
  final fakeRates = [
  ...
  // Assert - المفروض يرجع Right(rates)
  expect(result, Right(fakeRates));
});
```

**الشرح:**
- الـ Data Source نجح ورجع list of rates
- الـ Repository بيحطها في `Right(fakeRates)` (يعني نجح ✅)
...
### 1. Either<Failure, List<HistoricalRateEntity>>
```dart
Either<Failure, List<HistoricalRateEntity>>
```

**الشرح:**
- نوع بيانات بيحتوي على واحد من اثنين:
  - `Right(rates)`: نجح ✅ - فيه list of rates
  - `Left(failure)`: فشل ❌ - فيه error

**مثال:**
```dart
// Success
Right([HistoricalRateEntity(...), ...])  // ✅ نجح، فيه rates
...
│  يرجع: List<HistoricalRateModel> أو Exception
└──────────────┬──────────────────────┘
               │
               ▼
┌─────────────────────────────────────┐
...
### 2. Type Safety ✅
- نتأكد ان الـ return type صحيح: `Either<Failure, List<HistoricalRateEntity>>`
...
when(() => mockDataSource.getHistoricalRates('USD', 'EGY'))
    .thenAnswer((_) async => fakeRates);
```
- نخلي الـ Data Source يرجع rates صحيحة
- الـ Repository المفروض يحطها في `Right(fakeRates)`
...
```dart
expect(result, Right(fakeRates));  // Success
...
flutter test test/features/historical_rates/repository_test.dart
```
...
|-----------|--------|-------|
| ✅ Data Source نجح | ✅ نعم | Right(rates) |
